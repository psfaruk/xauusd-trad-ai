"""Signal lifecycle tracking (SPEC §8.4).

Live tracker: ticks drive SL/TP touches; M15 bar closes drive expiry.
- BUY exits on bid (SL: bid <= sl; TP: bid >= tp)
- SELL exits on ask (SL: ask >= sl; TP: ask <= tp)
- after `expiry_bars` M15 closes: expired with result_r = (close-entry)/risk
  signed by direction.
...
class TrackedSignal:
    ...
    trace: dict
    bars_seen: int = 0
    status: str = "active"  # active | won | lost | expired | cancelled
...
class SignalTracker:
    ...
    async def on_bar_close(
        ...
        """Count a closed engine-TF bar per active signal; expire when full."""
        async with self._lock:
            done: list[str] = []
            for sig in self._active.values():
                sig.bars_seen += 1
